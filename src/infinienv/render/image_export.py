...
def render_scene_image(
    ...
    *,
    ...
    for wall in scene.walls:
        draw.rectangle(cell_box(wall.x, wall.y), fill=COLORS["wall"], outline=GRID_LINE)
    ...
    ax0, ay0, ax1, ay1 = cell_box(*agent_pos)
    draw.ellipse((ax0 + 3, ay0 + 3, ax1 - 3, ay1 - 3), fill=COLORS["agent"], outline=(0, 0, 0))
    if inventory:
        draw.text((ax0 + 3, ay1 + 2), f"+{len(inventory)}", fill=(30, 100, 240), font=font)
...
def save_render_png(scene: SceneSpec, out_path: str, *, title: str | None = None) -> None:
    img = render_scene_image(scene, title=title)
    img.save(out_path)
