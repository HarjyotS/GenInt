...
# mode) as root. IS_SANDBOX=1 is meant to waive that, but it did NOT hold when the SDK spawns the CLI
# inside the container, so run as a non-root user — that sidesteps the root guard entirely (and is
# Anthropic's recommended way to run Claude Code in a container). Give it a real HOME (the CLI writes
# auth/config to ~/.claude) and ownership of /app so the app, the per-run sandbox workspaces under
# runs/, and the asset cache are all writable. IS_SANDBOX stays set as harmless belt-and-suspenders.
RUN useradd -m -u 10001 -s /bin/bash appuser \
    && mkdir -p /app/runs \
    && chown -R appuser:appuser /app
USER appuser
ENV HOME=/home/appuser IS_SANDBOX=1 PYTHONUNBUFFERED=1
# Persist runs/ with a NAMED volume (`-v infinienv_runs:/app/runs`) so it inherits appuser's
# ownership; a host bind-mount is owned by the host user and appuser (uid 10001) can't write to it.
EXPOSE 5050
